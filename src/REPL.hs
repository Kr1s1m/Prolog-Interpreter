...
import Types
    ( Command(..), Substitution, Program, Term, showSubstitution )
...
prologREPL = do
    ...
    if null program
    then
        putStrLn "Syntax error inside .pl file!!!"
    else 
...
--on next load of main function which requires reload of Main.hs to execute main again
...
welcomeMsg :: String
welcomeMsg = "Welcome to Prolog in Haskell version 2.1!\n" ++
...
-- Its arguments are a string to be written and next process p.
writeStr :: String -> (String -> String) -> (String -> String)
writeStr output p input = output ++ p input

-- Its argument is a process p which receives a line.
